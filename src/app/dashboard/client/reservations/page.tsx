                ...
                <div className="flex items-start gap-3">
                  <div className="h-10 w-10 shrink-0 overflow-hidden rounded-full bg-gray-100">
                    {coachData?.photo_url
                      ...
                      : <div className="flex h-full w-full items-center justify-center text-gray-300 text-sm">👤</div>}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <Link href={`/coachs/${coachData?.id}`} className="font-medium text-gray-900 hover:underline truncate">
                        {coachProfile?.nom ?? "Coach"}
                      </Link>
                      <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium ${statutStyle[r.statut]}`}>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      ...
                      {dispo && <span className="ml-2 capitalize">· {dispo.heure_debut.slice(0, 5)}–{dispo.heure_fin.slice(0, 5)}</span>}
                    </p>
